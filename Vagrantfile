...
Vagrant.configure("2") do |config|
  ...
  config.trigger.after :environment_unload, type: :hook do |t|
    t.ruby do |env, machine|
      snapshot_machine_name_list.each do |name|
        puts "Create first snapshot from machine '#{name}' with name '#{SNAPSHOT_FIRST_INIT}' (if not exist)"

        command = "vagrant snapshot list '#{name}' | grep -q '#{SNAPSHOT_FIRST_INIT}' || vagrant snapshot save '#{name}' '#{SNAPSHOT_FIRST_INIT}'"
        system(command)
      end
    end
  end
  ...
  config.vm.provision "file", source: "./vagrant-kind-config.yaml", destination: "/tmp/kind-config.yaml"
  config.vm.provision "shell" do |s|
  ssh_pub_key_file = "/tmp/id_rsa.pub"
  s.inline =
    <<-SHELL
      sed -i -E "s/#?DNSSEC=.*/DNSSEC=no/g" /etc/systemd/resolved.conf
      systemctl restart systemd-resolved
      ...
      echo "deb [arch=$(dpkg --print-architecture) signed-by=/etc/apt/keyrings/helm-apt-keyring.gpg] https://baltocdn.com/helm/stable/debian/ all main" | tee /etc/apt/sources.list.d/helm-stable-debian.list > /dev/null
      apt-get update
